...
# [omnia] background process to configure grafana
configure() {
    echo "Waiting for Grafana to be ready..."
    until $(curl --output /dev/null --silent --fail http://admin:admin@localhost:3000); do
        echo "Grafana is not ready yet, retrying configuration in 5 seconds"
        sleep 5
    done
